...
public class FacturaCompraController implements ActionListener {
    ...
    private void addProductoToDetalle() throws MessageException {
        ...
        try {
            precioUnitario = JGestionUtils.setScale(jdFactura.getTfPrecioUnitario());
        ...
        try {
            if (!jdFactura.getTfProductoDesc().getText().trim().isEmpty()) {
                descuento = JGestionUtils.setScale(jdFactura.getTfProductoDesc());
        ...
        if (descuento.signum() == 1) {
            if (tipoDescuento == 0) {
                precioUnitario = JGestionUtils.setScale(precioUnitario.subtract(precioUnitario.multiply((descuento.divide(new BigDecimal("100"))))));
            } else {
    ...
    void refreshResumen() {
        ...
        if (tipo.equalsIgnoreCase("A") || tipo.equalsIgnoreCase("M")) {
            for (int i = (dtm.getRowCount() - 1); i > -1; i--) {
                ...
                if (new BigDecimal(iva).compareTo(BigDecimal.ZERO) == 0) {
                    ...
                    otrosIVA = otrosIVA.add(subTotalSinIVA);
                    ivas = ivas.add(UTIL.getPorcentaje(subTotalSinIVA, new BigDecimal(iva)));
                }
            }
        ...
        jdFactura.setTfGravado(UTIL.PRECIO_CON_PUNTO.format(gravado21.add(gravado105).add(otrosIVA)));
        ...
        jdFactura.getTfTotalOtrosIVA().setText(UTIL.PRECIO_CON_PUNTO.format(ivas));
...
//        jdFactura.getTfTotalImpuestosNoRecuperables().setText(UTIL.PRECIO_CON_PUNTO.format(noRecuperables));
        jdFactura.setTfTotal(UTIL.PRECIO_CON_PUNTO.format(gravado21.add(gravado105).add(otrosIVA).
    ...
    public void initBuscador(Window frame, final boolean modal, final boolean toAnular) throws MessageException {
        ...
        UTIL.getDefaultTableModel(
                ...
                new Class<?>[]{null, null, Integer.class, null, null, String.class, null, null, null, Date.class});
    ...
    void initComprobanteUI(Window owner, boolean modal, boolean loadData) {
        ...
        jdFactura.getjTable1().getColumnModel().getColumn(4).setCellRenderer(JGestionUtils.getCurrencyRenderer());
...
//            newFacturaCompra.setImpuestosNoRecuperables(BigDecimal.ZERO);
        ...
        newFacturaCompra.setOtrosIvas(new BigDecimal(jdFactura.getTfTotalOtrosIVA().getText().trim()));
